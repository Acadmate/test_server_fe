...
export default function AppLoadingManager({
  children,
}: AppLoadingManagerProps) {
  const [isAppReady, setIsAppReady] = useState(false);
  const [showSplash, setShowSplash] = useState(true);

  useEffect(() => {
    try {
      // Skip splash screen in development mode
      ...
      if (!isPWA || hasVisitedBefore) {
        // Skip splash screen for regular web visits or returning users
        setShowSplash(false);
        setIsAppReady(true);
      } else {
        // Show splash screen only for first PWA launch
        setShowSplash(true);
        // Mark as visited
